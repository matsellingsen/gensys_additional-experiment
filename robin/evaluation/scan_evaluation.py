...
def evaluate_sequence_accuracy(model, dataloader, device, use_oracle=False):
    ...
    for source_sequences, _, target_output in dataloader:
        ...
        predictions = greedy_decode(model, source_sequences, target_output_lengths=target_output_lengths, use_oracle=use_oracle)
...
@torch.no_grad()
def evaluate_ordered_token_accuracy(model, dataloader, device, use_oracle=False):
    model.eval()
    total_acc = 0.0
    steps = 0
    ...
    for source_sequences, _, target_output in dataloader:
        ...
        predictions = greedy_decode(model, source_sequences, use_oracle)
...
def teacher_forced_ordered_sequence_accuracy(
    logits,
    targets,
    pad_idx=PAD,
    eos_idx=EOS,
    ...
    """
    Ordered, position-wise token accuracy using teacher forcing.
    Partial credit, sequence-aware, single forward pass.
...
@torch.no_grad()
def evaluate_teacher_forced_ordered_token_accuracy(model, dataloader, device):
    model.eval()
    total_acc = 0.0
    steps = 0
    ...
    for src, tgt_in, tgt_out in dataloader:
        ...
        logits = model(src, tgt_in)  # ONE forward pass
        acc = teacher_forced_ordered_sequence_accuracy(logits, tgt_out)
